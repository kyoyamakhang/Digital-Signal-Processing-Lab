...
function [yn, yorigin] = circular_conv_mod(xn, hn)
    N = length(xn);
    yn = zeros(1, N);
    for n = 0 : N-1
        sum_val = 0;
        for k = 0 : N-1
            h_idx = pmodulo(n - k, N) + 1;
            x_idx = k + 1;
            sum_val = sum_val + xn(x_idx) * hn(h_idx);
        end
        yn(n + 1) = sum_val;
    end
    yorigin = 0;
...
hn = [1 0 -1 -1 1];
horg = 0;
[yn, yorg] = circular_conv_mod(xn, hn);
